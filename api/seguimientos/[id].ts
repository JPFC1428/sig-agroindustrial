...
export default async function handler(
  ...
  const id = getSeguimientoIdFromRequestUrl(request.url);
  ...
  try {
    ...
    await handleSeguimientoItem(request, response, id);
  } catch (error) {
    const detail =
    ...
    if (!response.headersSent) {
      ...
      response.end(
        JSON.stringify({
          error: "Error interno en /api/seguimientos/[id]",
          detail,
        })
      );
      return;
