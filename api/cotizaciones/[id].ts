...
export default async function handler(
  ...
  const id = getCotizacionIdFromRequestUrl(request.url);
  ...
  try {
    ...
    await handleCotizacionItem(request, response, id);
  } catch (error) {
    const detail =
    ...
    if (!response.headersSent) {
      ...
      response.end(
        JSON.stringify({
          error: "Error interno en /api/cotizaciones/[id]",
          detail,
        })
      );
      return;
